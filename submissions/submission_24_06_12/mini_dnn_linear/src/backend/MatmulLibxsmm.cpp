                                                     ...
                                                     at::Tensor i_w)
{
  ...
  l_kernel_forward.gemm = libxsmm_dispatch_brgemm(l_shape_brgemm,
                                                  l_flags_brgemm,
...
#pragma omp parallel for collapse(2) private(l_param)
  for (int64_t l_kb = 0; l_kb < l_sizes.kb; l_kb++)
  {
    for (int64_t l_nb = 0; l_nb < l_sizes.nb; l_nb++)
    {
      int64_t l_offset_c = l_kb * l_strides_c[0] + l_nb * l_strides_c[1];

      for (int64_t l_cb = 0; l_cb < l_sizes.cb; l_cb++)
      {
        int64_t l_offset_a = l_nb * l_strides_a[0] + l_cb * l_strides_a[1];
        int64_t l_offset_b = l_kb * l_strides_b[0] + l_cb * l_strides_b[1];
